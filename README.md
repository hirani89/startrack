...
Installation is very easy with composer:
    ...
    composer require zoet/startrack
...
```
<?php
	$startrack = new \Zoet\Startrack\Startrack('Your API Key', 'Your API Password', 'Your Account Number', $testmode);

	// Get your account details
	$account = $startrack->getAccountDetails();

	// Create a shipment

	$shipment = $startrack->newShipment()
		->setFrom(new \Zoet\Startrack\Address([
			'first_name' => 'Joe',
			'last_name' => 'Tester',
			// OR 'name' => 'Joe Tester',
			'lines' => [
		...
		->setTo(new \Zoet\Startrack\Address([
			'first_name' => 'Mary',
			'last_name' => 'Tester',
			'lines' => [
		...
		->addParcel(new \Zoet\Startrack\Parcel([
			'item_reference' => 'pkg1',
			'length' => 5,
			'height' => 4,
			'width' => 45,
	...
	$label_pdf = $startrack->getLabels($shipment_ids, new \Zoet\Startrack\LabelType([
		'layout_type' => \Zoet\Startrack\LabelType::LAYOUT_A4_FOUR_PER_PAGE,
		'branded' => false,
	]));
